...
if ( ! function_exists( 'ambulance_posted_on' ) ) :
	...
	function ambulance_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'ambulance' ),
...
if ( ! function_exists( 'ambulance_posted_by' ) ) :
	...
	function ambulance_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'ambulance' ),
	...
	function ambulance_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'ambulance' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'ambulance' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'ambulance' ),
					array(
						'span' => array(
							'class' => array(),
						),
